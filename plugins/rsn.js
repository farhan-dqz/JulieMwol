...
if (Config.WORKTYPE == 'private') {
    ...
    Asena.addCommand({ pattern: 'rsn ?(.*)', fromMe: true,dontAddCommandList: true }, (async (message, match) => {
    ...
    Asena.addCommand({ pattern: 'rsn ?(.*)', fromMe: false,dontAddCommandList: true}, (async (message, match) => {

        if (match[1] === '') return await message.sendMessage(need);

        var ttinullimage = await axios.get(`https://api.zeks.xyz/api/snowwrite?apikey=cjxJIn9eDletRHac3CG6CdvGxFH&text1=SOPHIA%20SL&text2=${encodeURIComponent(match[1])}`, { responseType: 'arraybuffer' })
