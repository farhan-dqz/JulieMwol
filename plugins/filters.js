...
Asena.addCommand({on: 'text', fromMe: false}, (async (message, match) => {
        ...
        if(banned !== undefined) return
        if (!!message.mention && message.mention[0] == '918921483992@s.whatsapp.net') {
await message.client.sendMessage(message.jid, fs.readFileSync('./stickers/mention.webp'), MessageType.sticker, { mimetype: Mimetype.webp, quoted : message.data, ptt: true})
        }
const array = ['Hi','power']
array.map( async (z) => {
let pattern = new RegExp(`\\b${z}\\b`, 'g');
if(pattern.test(message.message)){
       await message.client.sendMessage(message.jid, fs.readFileSync('./stickers/' + z + '.webp'), MessageType.sticker, { mimetype: Mimetype.webp, quoted: message.data, ptt: true})
}
});
    }
